...
var PraisewallModel = function() {
	...
	*/
    ...
    /**
        retrieves other praise_wall on scroll down
    */
    this.reapNextFeeds = function (dataString) { 
		$.ajax({
            type: "GET",
            url: "../praise_wall/get_praise_wall_ajax",
            data: dataString,
            cache: false,
            success: function(response) {
				$(".loading").hide();	
                response = JSON.parse(response);
                $("#praisewall-last-updated").val(response['praisewall_last_updated']);
...
var PraisewallHelper = function(){
	this.generateSnsProfilePicLink = function(username) {
		return "http://graph.facebook.com/" + username + "/picture?";
	}
}

var PraisewallController = function() {
    
	var m = new PraisewallModel(),
		h = new PraisewallHelper();
    ...
    this.onClick = function() {     
		...
		*/
		$('.form_feed').click(function(e) {
			...
			else if (self.prevPraisePost != message) {
	...
	/**
		requests for more praise_wall
	*/
	this.onScroll = function() {
		$(window).scroll(function() {
			if($(window).scrollTop() == $(document).height() - $(window).height() && !BUSY) {
				BUSY = true;
                var lastUpdated = $("#praisewall-last-updated").val();
                
                if(lastUpdated != "REAPED_LAST") {
                    dataString = 'praisewall_last_updated=' + lastUpdated;
